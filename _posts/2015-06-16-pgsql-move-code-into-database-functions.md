...
> 原文引用：[https://sivers.org/pg](https://sivers.org/pg)
...
* 所有东西都必须通过 Ruby/Python/PHP/Javascript 类中－也包括 shell 脚本以及其他不属于网站的部分；
* 所有东西都不能直接访问数据；为了这么做，你必须要把逻辑定义在周边的类中；
* 数据被当作愚蠢的存储，即使数据库足够智能，可以完成大部分这些逻辑；
...
但是 **这整个事件，我信任的 PostgreSQL 数据库都是不变的**。

因为大部分都是数据逻辑，而不是业务逻辑，所以它应该在数据库中。
...
我并不打算舒服所有人都这么做。但是我希望对你有用处或者至少有趣。
